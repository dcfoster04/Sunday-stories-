    ...
    <section className="bg-ink-950 bg-grain py-24 text-paper-100 sm:py-32">
      <Container size="default">
        <Reveal className="mx-auto max-w-2xl text-center">
          <p className="kicker text-flare-400">Persistent league memory</p>
          <h2 className="mt-4 text-balance font-display text-4xl font-black uppercase leading-[1.02] tracking-tighter sm:text-5xl">
            ...
            <div className="grid grid-cols-1 gap-8 sm:grid-cols-5 sm:gap-4">
              {TIMELINE.map((step, i) => (
                <div key={step.label} className="relative flex flex-col items-start sm:items-center sm:text-center">
                  <div className="relative z-10 flex h-8 w-8 items-center justify-center rounded-full border-2 border-flare-400 bg-ink-950 font-display text-xs font-bold text-flare-400">
                    {i + 1}
                  </div>
        ...
        <Reveal delay={200} className="mt-16 text-center">
          <p className="font-serif text-2xl italic text-flare-300 sm:text-3xl">
